...
import cherrypy
import urllib, re, os.path, datetime
import i18n
import logging
from lib.util import make_url, use_future_expires
        ...
        if filename:
            ...
            for fn in fnlist:
                if fn.endswith('.css'):
                    ...
                    try:
                        cherrypy.lib.static.serve_file(fn)
                        return True
                    except cherrypy.NotFound:
                        # fall back to the original staticdir handler if we're out of filenames to try
                        pass
    ...
    if not handled:
        ...
        if not handled and generate_indexes:
            if callable(generate_indexes):
                handled = cherrypy.lib.static.generate_indexes(filename, branch)
            else:
