...
#
querylogger = logging.getLogger('splunk.searchhelper')
querylogger.setLevel(logging.DEBUG)
querylogger.propagate = False
searchlogpath = os.path.join(os.environ['SPLUNK_HOME'], 'var', 'log', 'splunk', 'searches.log')
# Keep at most 22MB of searches.log (30 days in an extreme environment) and 5 backup/rotated. Without this limitation,
# searches.log can get too large for the max file size on the OS and gets impractical for the search assistant to search.
queryhandler = logging.handlers.RotatingFileHandler(searchlogpath, maxBytes=24000000, backupCount=5)
queryhandler.setFormatter(logging.Formatter("%(asctime)s - %(message)s"))
querylogger.addHandler(queryhandler)
...
class JobsController(BaseController):
    ...
    def getJobAsset(self, sid, asset, **kwargs):
        ...
        if outputMode == 'json': ct = MIME_JSON
...
class SearchController(BaseController):
    ...
    def typeahead(self, q, index='main', namespace=None, earliest_time=None, latest_time=None, count=None, max_time=None, servers=None, **kwargs):
        ...
        try:
            ...
            for item in output:
                item['count'] = i18n.format_number(int(item['count']))
