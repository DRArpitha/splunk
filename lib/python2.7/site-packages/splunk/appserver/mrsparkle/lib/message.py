...
def get_session_queue():
    ...
    if sess.has_key(QUEUE_SESSION_KEY):
...
def save_to_session(fn, self, *a, **kw):
    ...
    if (hasattr(cherrypy, 'session')):
        # let the session handler know that we're changing something about the queue object
        if hasattr(cherrypy.session, 'changed'):
            cherrypy.session.changed = True
        cherrypy._test_session_has_changed = True
        newq = cherrypy.session.get(QUEUE_SESSION_KEY)
        if not newq:
            # session was toasted while we were working
            cherrypy.session[QUEUE_SESSION_KEY] = self
            self.queue = []
        elif newq!=self:
            # queue may have changed while we were working
            # use the latest queue before we start making changes to it
            self.queue = newq.queue
            cherrypy.session[QUEUE_SESSION_KEY] = self
...
if __name__ == '__main__':
    ...
    class SessionQueueTests(unittest.TestCase):
        ...
        def tearDown(self):
            ...
            self.assert_(hasattr(cherrypy, 'session') is False)
