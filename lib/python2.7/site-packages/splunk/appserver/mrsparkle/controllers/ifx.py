    ...
    def getTestURL(self, regex, args):
        if len(regex) > 0:
            ...
            if len(xfields) > 0:
                ...
                regex = regex.replace('\\', '\\\\')
                regex = regex.replace('"', '\\"')
                # return self.make_url('/app/search/flashtimeline',  _qs=[('q', 'search %s | head 10000 | rex "%s" | fields %s | dedup %s' % (args['samplesearch'], regex, xfieldslist, xfieldslist))])                
                return self.make_url('/app/search/flashtimeline',  _qs=[('q', 'search %s | head 10000 | rex "%s" | top 50 %s' % (args['samplesearch'], regex, xfieldslist))])
...
def generateOutput(args, events, fullevents, regex, testingurl):
    ...
    try:
        ...
        unnamedExtractions = re.search("(?:^|[^\\\])\((?!\?)", regex)

        if unnamedExtractions != None:
            start = unnamedExtractions.start()            
            raise Exception('no named extraction at position %s (i.e., "%s...").  Expected "(?P<variable>pattern)"' % (start, regex[start:start+10]))
