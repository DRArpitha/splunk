...
def doHelp(sessionKey, namespace, user, search, insertpos=None, earliest_time=None, latest_time=None, count=10, max_time=None, servers=None,
    ...
    try:
        ...
        output['allcommands'] = utils.getAllCommands(bnf)
        ...
        if (splunk.util.normalizeBoolean(useTypeahead)):
            suggestSearchTypeahead(output, search, count, max_time, earliest_time, latest_time, servers, namespace, user)
...
def suggestSearchTypeahead(output, search, count, max_time, earliest_time, latest_time, servers, namespace, user):

    commandAndArgs = utils.getLastCommand(search, None)
    if commandAndArgs != None:
        command, args = commandAndArgs
        if command == "search":
            output['typeahead'] = getTypeaheadTerms(args, count, max_time, earliest_time, latest_time, servers, namespace, user)
...
def getTypeaheadTerms(q, count, max_time, earliest_time, latest_time, servers, namespace, user):
    typeaheadTerms = []
    ...
    if response != None and response.status == 200:
        ...
        for item in output:
            try:
                ...
                #    token = token[1:-1]
                replacement, match, fullmatch = getReplacement(q, token)
...
def nextCommand(output, sessionKey, namespace, user, search, usersquery, queryprefix, aliasMap, bnf, showargs):
    ...
    if commandAndArgs == None:
        ...
        typeaheadcommands = list(utils.getAllCommands(bnf, True))
        ...
        if command in aliasMap:
            ...
            for thiscommand in utils.getAllCommands(bnf):
