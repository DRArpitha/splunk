...
CMD_ENDPOINT_MAP = {
                   ...
                   '*:discoverable':'distributed-search',
                   '*:deploy-server':'deployments',
                   '*:deploy-clients':'deployments',
                   '*:deploy-client':'deployments',
                   '*:deploy-multicast':'deployments',
                   '*:server-class':'deployments',
                   '*:deploy-poll':'deployments', 
                   '*:exec':'scripted',
                   '*:scripted':'scripted',
                   'display:listen':'receiver',
                   'enable:listen':'receiver',
                   'disable:listen':'receiver',
...
WHITELIST = {
              'set': ['deploy-multicast', 'datastore-dir', 'default-hostname', 'default-index', 'servername', 'minfreemb', 'server-type', 'splunkd-port', 'web-port', 'deploy-poll'],
              'show': ['deploy-multicast', 'datastore-dir', 'default-hostname', 'default-index', 'servername', 'minfreemb', 'server-type', 'splunkd-port', 'web-port', 'jobs', 'auth-method', 'config', 'license', 'deploy-poll'],
            }
...
def makeRestCall(cmd=None, restArgList=None, obj=None, sessionKey=None, entityName=None, search=None, count=None, offset=None, sort_key=None, sort_dir=None, dotSplunk=None):
   ...
   #hack for reload deploy-server command, users a very different conditional url structure
   if (cmd in ['list','refresh'] and obj in ['server-class', 'deploy-clients']) or (cmd in ['display','enable','disable']  and obj == 'deploy-server'):
      if not argList.has_key('tenant'):
         argList['tenant'] = 'default'
   elif cmd == 'reload' and obj == 'deploy-server':
      if argList.has_key('class'):
         argList['tenant_or_class1'] = argList['class']
         if argList.has_key('tenant'):
            argList['tenant_or_class2'] = '%s.%s.Reload' % (argList['tenant'], argList['class'])
         else:
            argList['tenant_or_class2'] = 'default.%s.Reload' % (argList['class'])
      else:
         if argList.has_key('tenant'):
            argList['tenant_or_class1'] = argList['tenant']
            argList['tenant_or_class2'] = '%s.Reload' % argList['tenant']
         else:
            argList['tenant_or_class1'] = 'default'
            argList['tenant_or_class2'] = 'default.Reload'
   ...
   #hack for removing all async jobs, to be removed when an EAI endpoint is provided to do this
     ...
     return handleRemoveJobsAll(sessionKey, namespace, owner)
      ...
      #this essentially means %(name)s % argList i.e the contents of key 'name' in dict argList
         ...
         try:
            eai_id = eval(" '%s' % lf_eai_id % argList ")
...
if __name__ == '__main__':
   ...
   class MakeRestCallTests(unittest.TestCase):
      ...
      # ----------------------------------------
      def testMakeRestCall_show_web_port(self):
         """
         show web-port
         """
         retVal = makeRestCall(cmd='show', obj='web-port', restArgList={}, sessionKey=self.sessionKey)
         self.assertTrue(retVal != '')

      # ----------------------------------------------
      def testMakeRestCall_show_splunkd_port(self):
         """
         show splunkd-port
         """
         retVal = makeRestCall(cmd='show', obj='splunkd-port', restArgList={}, sessionKey=self.sessionKey)
         self.assertTrue(retVal != '')
      ...
      # ------------------------------------------
      def testMakeRestCall_show_minfreemb(self):
         """
         show minfreemb
         """
         retVal = makeRestCall(cmd='show', obj='minfreemb', restArgList={}, sessionKey=self.sessionKey)
         self.assertTrue(retVal != '')

      # -------------------------------------------
      def testMakeRestCall_show_servername(self):
         """
         show servername
         """
         retVal = makeRestCall(cmd='show', obj='servername', restArgList={}, sessionKey=self.sessionKey)
         self.assertTrue(retVal != '')
