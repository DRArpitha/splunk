...
from splunk.appserver.mrsparkle.lib import viewconf, i18n
...
class MemoizedViews(object):
    ...
    def getAvailableViews(self, namespace, refresh, output):
        time1 = time.time()

        try:
            digests = en.getEntities(VIEW_ENTITY_CLASS, namespace=namespace, refresh=refresh, count=-1, digest=1)
        ...
        for item,digest in digests.iteritems():
            parsed_view = self.getParsedView(namespace, refresh, rawxml, item, digest.get('eai:digest'))
            if parsed_view:
    ...
    def _generateViewRoster(self, viewName, viewConfig, urlPath):
        ...
        output = {
            'layoutRoster': {}, 
            'activeModules': {}, 
            'label': viewConfig.get('label', viewName),
    ...
    def hasAutoRun(self, viewConfig):
        ...
        for module in viewConfig.get("modules", []):
            if module.has_key('params') and module['params'].get('autoRun', False): 
                hasAutoRun = True
                break
        return hasAutoRun
