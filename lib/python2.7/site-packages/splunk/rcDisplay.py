...
__doc__ = """
          ...
          """
__version__ = "1..0.0"
__copyright__ = """Copyright (C) 2005-2011 Splunk Inc. All Rights Reserved.  Version 4.0"""
...
TCP_RECEIVE         = '''Receiving is enabled on port %s.'''
TCP_RECEIVE_NONE    = '''Receiving is disabled.'''
TCP_RECEIVE_ADD     = '''Listening for Splunk data on TCP port %s.'''
TCP_RECEIVE_REMOVE  = '''No longer listening for Splunk TCP data.'''
TCP_RECEIVE_DISABLE = '''Receiving is disabled on port %s.'''
#help
...
FORWARD_SERVER_REMOVE_NONE1 = '''Cannot remove because Splunk is not currently forwarding to %s.'''
FORWARD_SERVER_REMOVE_NONE2 = '''Was unable to find server '%s' for removal.'''
FORWARD_SERVER_REMOVE = '''Stopped Splunk-2-Splunk forwarding to: %s'''
#deployment server/client
DEPLOYMENT_SERVER_NO_SERVER_CLASS = '''This deployment client is not a member of any classes.'''
DEPLOYMENT_SERVER_CLASS = '''This deployment client is a member of the following classes:'''
...
RELOAD_DEPLOY_SERVER       = '''Reloading server classes (and checking for any new or removed classes).'''
REFRESH_DEPL_CLIENTS       = '''Refreshing deployment client list.'''
...
AUTH_METHOD_REMOVE_NONE = '''The auth method '%s' does not currently exist, and thus cannot be removed.'''
AUTH_METHOD_REMOVE_FAIL = '''The authentication method currently in use can not be removed.'''
...
DIST_SEARCH_LIST = '''Server at URI "%s" with status as "%s" and peerType "%s"'''
DIST_SEARCH_LIST_NONE = '''(no servers)'''
DISPLAY_DISCOVERABLE_ENABLED = '''Discoverable mode: enabled.'''
DISPLAY_DISCOVERABLE_DISABLED = '''Discoverable mode: disabled.'''
DISABLE_DISCOVERABLE = '''Discoverable mode is now disabled.'''
ENABLE_DISCOVERABLE = '''Discoverable mode is now enabled.'''
...
#listen
DISABLE_LISTEN = 'Port %s is not a current listen port.'
#no info
...
def displayResourceError(**kwargs):
   ...
   if kwargs['obj'] in ['monitor', 'tail']:
   ...
   elif kwargs['obj'] == 'listen' and kwargs['cmd'] == 'disable':
       logger.error(DISABLE_LISTEN % kwargs['uri'].split('/')[-1])
      ...
      logger.error('Splunkd server REST interface returned an error: cmd: %s, obj: %s, uri: %s, error_text: %s. Please file a case online at http://www.splunk.com/page/submit_issue' % (kwargs['cmd'], kwargs['obj'], kwargs['uri'], kwargs['serverContent']))
...
def displaySettings(**kwargs): 
      ...
      if kwargs['obj'] == 'license':
         ...
         if kwargs['obj'] == 'web-port':
            print SHOW_WEBPORT % extractLevel1Feed(serverContent=kwargs['serverContent'], filter=['httpport'])
         elif kwargs['obj'] == 'splunkd-port':
            print SHOW_SPLUNKDPORT % extractLevel1Feed(serverContent=kwargs['serverContent'], filter=['mgmtHostPort'])
         elif kwargs['obj'] == 'default-hostname':
            print SHOW_HOSTNAME % extractLevel1Feed(serverContent=kwargs['serverContent'], filter=['host'])
         elif kwargs['obj'] == 'minfreemb':
            print SHOW_MINFREEMB % extractLevel1Feed(serverContent=kwargs['serverContent'], filter=['minFreeSpace'])
         elif kwargs['obj'] == 'servername':
            print SHOW_SERVERNAME % extractLevel1Feed(serverContent=kwargs['serverContent'], filter=['serverName'])
         ...
         if kwargs['obj'] == 'default-hostname':
            print SET_HOSTNAME
            print RESTART_SPLUNK
         elif kwargs['obj'] in ['web-port', 'minfreemb', 'servername']:
            print RESTART_SPLUNK
         elif kwargs['obj'] == 'splunkd-port':
            print SET_SPLUNKDPORT
            print RESTART_SPLUNK
         elif kwargs['obj'] == 'datastore-dir':
...
def displayHelp(**kwargs):
   ...
   try:
      print eval('literals.%s' % kwargs['help_text'])
   except SyntaxError:
