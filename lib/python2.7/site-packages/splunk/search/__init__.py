...
def dispatch(search, hostPath=None, sessionKey=None, namespace=None, owner=None, **kwargs):
    '''
...
def getJob(sid, hostPath=None, sessionKey=None, message_level=None):
    '''Returns a SearchJob object for a specific job.'''
    return SearchJob(sid, hostPath, sessionKey, message_level=message_level)    
...
class SearchJob(object):
    ...
    def __init__(self, searchId, hostPath, sessionKey=None, namespace=None, owner=None, message_level=None, dispatchArgs={}):
    ...
    def _execControl(self, action, kwargs=None):
        ...
        if serverResponse.status != 200:
            ...
            util.extractMessages(et.fromstring(serverContent))
            return False
    ...
    def _getStatus(self, isInitial=False, force=False):
        ...
        stateRetryCount = 0
        while (time.time() - loopStartTime) < STATUS_FETCH_TIMEOUT:
            ...
            # job is created and can be polled
                ...
                if dispatchEnum < PASSABLE_DISPATCH_ENUM:
...
class RawEvent(object):
    ...
    def __init__(self, plainString=None):
...
class JobLite:
    ...
    def get(self, asset):
        ...
        uri = '/services/search/jobs/%s/%s' % (self.sid, asset)
...
if __name__ == '__main__':
    ...
    class SearchResultTests(unittest.TestCase):
        ...
        def testFeedEvent(self):
            ...
            self.assertEquals(len(feed), rowcount)
            ...
            self.assertEquals(len(feed), count)
            
            for i in range(offset, offset + count - 1):
                self.assertEquals(int(feed[i - offset]['position']), i)
        ...
        def testFeedCount(self):
            ...
            feed = eval(feed)
            self.assertEquals(len(feed), 100)

            # check that count is respected
            job.setFetchOption(count=124)
            feed = job.getFeed('results')
            feed = eval(feed)
            self.assertEquals(len(feed), 124)
    ...
    class SearchMessaging(unittest.TestCase):
        ...
        def testMissingFieldCommand(self):
            '''
            Execs a search that has events, but followed by a search that expects a
            missing field
            '''
            
            j = dispatch('windbag | timechart count(bad_field)', sessionKey=self.sessionKey)
            waitForJob(j)

            # see SPL-36293
            j.message_level = 'debug'
            j._getStatus(force=True)

            self.assert_('debug' in j.messages)

            self.assertEquals(j.messages['debug'][0], "Specified field(s) missing from results: 'bad_field'")
            self.assert_(not j.isFailed)
    ...
    class SearchJobIterator(unittest.TestCase):
        ...
        def testAutoEventSelectionRunning(self):
