...
def didYouMean(command):
    ...
    commands = ["search", "login", "logout", "spool", "start", "stop", "restart", "status", "add", "edit", "list", "import", "export", "clean", "summary", "enable", "disable", "display", "train", "test", "validate", "anonymize", "find", "help", "diag", "dispatch", "getconf", "install", "migrate", "package", "recover", "refresh", "reload", "remove", "set", "show", "spool", "upgrade"]
...
cList.addCmd(SplunkCmd("add",    "server-class",     None, "class", removed = True))
...
cList.addCmd(SplunkCmd("remove", "server-class",     None, "class", removed = True))
...
cList.addCmd(SplunkCmd("disable", "listen",          None))
...
cList.addCmd(SplunkCmd("display", "listen",          None))
...
cList.addCmd(SplunkCmd("enable",  "listen",          None, "source"))
...
#SPL-28146
#cList.addCmd(SplunkCmd("_internal", "http",              ms.setLocalHttp, "port", authReq = False))
#cList.addCmd(SplunkCmd("_internal", "https",             ms.setLocalHttps, "port", authReq = False))
#cList.addCmd(SplunkCmd("_internal", "mgmt",              ms.setLocalMgmtPort, "port", authReq = False))
...
cList.addCmd(SplunkCmd("reload",  "deploy-server",   None))
...
cList.addCmd(SplunkCmd("set",  "deploy-multicast",   ca.set_depMcast, "uri", authReq = False))
...
cList.addCmd(SplunkCmd("set",  "minfreemb",          None, "minspace"))
cList.addCmd(SplunkCmd("set",  "servername",         ca.setServerName, "instancename", authReq = False))
cList.addCmd(SplunkCmd("set",  "web-port",           None, "httpport"))
cList.addCmd(SplunkCmd("set",  "web-version",        ca.setUIVersion, "version"))
cList.addCmd(SplunkCmd("set",  "splunkd-port",       None, "mgmturl"))
...
cList.addCmd(SplunkCmd("show", "deploy-multicast",   ca.get_depMcast, authReq = False))
cList.addCmd(SplunkCmd("show", "deploy-poll",        ca.get_depPoll, authReq = False))
cList.addCmd(SplunkCmd("show", "servername",         ca.get_servername, authReq = False))
...
def getAuthFilePath():
  try:
      ...
      hostname_list = hostname_ep.split('-')
      hostname = '-'.join(hostname_list[:len(hostname_list)-1])
  except:
...
def main(argsList):
  ...
  if len(argsList) < 2:
    ...
    #SPL-23125 - make error messages cleaner. Show more stuff only in debug mode
       ...
       if e.args[0].find('Connection refused') != -1:
